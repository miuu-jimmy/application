...
export default function ProfileInfo({wallet}) {
  const [profile, setprofile] = useState(Defaultprofile)
  const [nfts ,  setnfts] = useState([])
  const [show ,  setshow] = useState("hidden")
  const router = useRouter()
  const href="public-profile"
  function handlepp(e) {
    e.preventDefault()
    router.push(href)
  }
  useEffect(() => {
    const getNft = async (wallet)  => { 
      const nft =[]
      let res = await fetch('/api/getNfts?user='+wallet.publicKey.toString())
      let data = await res.json()
      console.log(data)
      setnfts(data.result.nfts) 
      return {data}
    }  
    getNft(wallet)
